...
class dus
{
    ll no,tem1,tem2,clusterno; vpll a;d dis;vpddd b;// d t1;
    pdd t2; d ans; vll rank,parent;
    vd distance;ll target;
    public:
    void inp()
    {
        cin >> no;
        ...
        //printf("% 0.8f",ans);
        target = distance.size()-(clusterno-1);
        printf("% 0.10f",distance[target]);
    }
    ll finpar(ll x)
    {
        while(x!=parent[x])
